...
class Model_products extends CI_Model
{
		...
		public function getProductData($id = null)
	{
		...
		if($user_id == 1) {
			...
			$data = array();
			foreach ($all as $k => $v) {
				$store_ids = json_decode($v['store_id'], true) ?: [];
				if(empty($store_ids) || in_array($user_data['store_id'], $store_ids)) {
					$data[] = $v;
				}
			}
		...
		if($cat_id) {
			...
			if($user_id == 1) {
				...
				foreach($query->result_array() as $key => $value) {
					$category_ids = json_decode($value['category_id'], true) ?: [];
				...
				foreach ($query->result_array() as $k => $v) {
					$store_ids = json_decode($v['store_id'], true) ?: [];
					$category_ids = json_decode($v['category_id'], true) ?: [];
